...
public class ControlPanel extends JPanel {
    ...
    public ControlPanel(SimulationPanel sim, SceneRenderer rend,
        ...
        setPreferredSize(new Dimension(215, 600));
        setBackground(BG);
    ...
    private void build() {
        ...
        JLabel lblD = makeLabel("Spawn mỗi 1.5s");
        add(lblD);
        JSlider slD = makeSlider(2, 60, 15);
        slD.addChangeListener(e -> {
            double v = slD.getValue()/10.0;
            simPanel.setSpawnInterval(v);
        ...
        JLabel lblSpd = makeLabel("Tốc độ: 100 px/s");
        add(lblSpd);
        JSlider slSpd = makeSlider(20, 250, 100);
        slSpd.addChangeListener(e -> {
            network.setDefaultMaxSpeed(slSpd.getValue());
            lblSpd.setText("Tốc độ: " + slSpd.getValue() + " px/s");
        });
        addFull(slSpd);

        // ── Khoảng cách theo xe ───────────────────────────────────────────
        addSection("Khoảng cách giữ với xe trước");
        JLabel lblFD = makeLabel("Default: 2/3 chiều dài xe");
        add(lblFD);
        JSlider slFD = makeSlider(5, 60, 13);  // default ~2/3 * 20px ≈ 13
        slFD.addChangeListener(e -> {
            int d = slFD.getValue();
            lblFD.setText("Khoảng cách: " + d + " px");
            // Áp dụng cho tất cả xe đang chạy
        ...
        JLabel lblGreen = makeLabel("Thời gian XANH: 8s");
        add(lblGreen);
        JSlider slGreen = makeSlider(2, 30, 8);
        slGreen.addChangeListener(e -> {
            int t = slGreen.getValue();
            lblGreen.setText("Thời gian XANH: " + t + "s");
            ics.forEach(ic -> ic.setGreenTime(t));
        });
        addFull(slGreen); addGap(3);

        JLabel lblYellow = makeLabel("Thời gian VÀNG: 2.5s");
        add(lblYellow);
        JSlider slYellow = makeSlider(1, 10, 3);  // *0.5 → 0.5..5s
        ...
        // ── Phím tắt ──────────────────────────────────────────────────────
        addSection("Phím tắt");
        addInfo("[Space] Tạm dừng");
        addInfo("[R]     Reset");
        addInfo("[S]     Sinh 1 xe");
        addInfo("[F]     Fit camera");
        addInfo("[WASD]  Pan");
        addInfo("[+/-]   Zoom");
        addInfo("[Scroll] Zoom tại cursor");
        addInfo("[Drag-P] Pan chuột");

        add(Box.createVerticalGlue());
        addGap(4);
        JLabel credit = new JLabel("Traffic Sim v4.0");
    ...
    private void addFull(JComponent c) {
        ...
        c.setMaximumSize(new Dimension(10000, c.getPreferredSize().height));
        add(c);
    }
